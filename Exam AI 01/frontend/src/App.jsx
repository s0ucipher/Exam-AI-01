...
function App() {
  ...
  const handleOrganize = async ({ sources, destinationPath, sortBy, applyChanges }) => {
    ...
    try {
      const response = await axios.post(`${API_BASE_URL}/api/organize`, {
        sources,
        destinationPath,
        sortBy,
        applyChanges,
  ...
  const handleSaveToDownloads = (data) => saveOrganized({
    data,
    saveMode: 'downloads',
...
function ResultsView({ data, onSaveToChosenLocation, onSaveToDownloads, saveResult, savingAction }) {
    ...
    <section className="results-grid">
      <div className="result-banner">
        <div>
          ...
          <strong>{data.destination_path || data.path}</strong>
          <small>{data.sources?.length || 0} source(s) scanned</small>
        </div>
        <span className={data.applied_changes ? 'applied-pill' : 'preview-pill'}>
          {data.applied_changes ? 'Files moved' : 'Preview mode'}
        </span>
      </div>

      <DashboardCards dashboard={data.dashboard} appliedChanges={data.applied_changes} />
...
function SavePanel({ data, onSaveToChosenLocation, onSaveToDownloads, saveResult, savingAction }) {
  return (
    <div className="save-panel">
      <div className="save-copy">
        <p>s0ucipher export</p>
        <h3>Save a clean organized copy after preview.</h3>
        <span>Original files stay where they are. NeuroSort copies everything into category folders.</span>
      </div>
...
function DashboardCards({ dashboard, appliedChanges }) {
  const cards = [
    { label: 'Total Files', value: dashboard?.total_files ?? 0, icon: Files, accent: 'blue' },
    { label: 'Sources', value: dashboard?.source_count ?? 0, icon: FolderTree, accent: 'violet' },
  ...
  return (
    <div className="dashboard-cards">
      ...
      <div className={`mode-card ${appliedChanges ? 'applied' : ''}`}>
        ...
        <span>{appliedChanges ? 'Live organize' : 'Safe preview'}</span>
      </div>
    </div>
  )
}
